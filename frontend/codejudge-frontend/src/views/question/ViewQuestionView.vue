      ...
      <a-col :md="12" :xs="24">
        <a-tabs default-active-key="question">
          ...
          <a-tab-pane key="comment" title="Discussion" disabled
            >Discussion is not available yet.</a-tab-pane
          >
          <a-tab-pane key="answer" title="Answer"
            >The reference answer is hidden from regular users.</a-tab-pane
          >
        </a-tabs>
      </a-col>
      <a-col :md="12" :xs="24">
...
const loadData = async () => {
  ...
  if (res.code === 0) {
    ...
    message.error(`Failed to load the problem: ${res.message}`);
  }
};
...
const doSubmit = async () => {
  ...
  if (res.code === 0) {
    message.success(
      "Submission created successfully. Check the submissions page for the result."
    );
  } else {
